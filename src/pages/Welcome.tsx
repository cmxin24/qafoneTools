import { useI18n } from '@/i18n';
import { MessageCircle, ExternalLink } from 'lucide-react';
import { cn } from '@/lib/utils';

const GITHUB_URL = 'https://github.com/cmxin24/qafoneTools';
function GitHubIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className} aria-hidden="true">
      <path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0 1 12 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z" />
    </svg>
  );
}

export default function WelcomePage() {
  const { t } = useI18n();
  const w = t.welcome;

  return (
    <div className="flex h-full flex-col items-center justify-center overflow-y-auto px-6 py-12">
      <div className="flex w-full max-w-lg flex-col items-center gap-8">

        {/* Hero */}
        <div className="flex flex-col items-center gap-4 text-center">
          <div>
            <h1 className="text-2xl font-bold tracking-tight">{w.title}</h1>
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground max-w-sm">
              {w.subtitle}
            </p>
          </div>
        </div>

        {/* Info cards */}
        <div className="grid w-full gap-3 sm:grid-cols-2">
          {/* GitHub */}
          <a
            href={GITHUB_URL}
            target="_blank"
            rel="noopener noreferrer"
            className={cn(
              'flex items-start gap-3 rounded-xl border border-border bg-card p-4',
              'transition-all duration-150 hover:border-primary/40',
            )}
          >
            <GitHubIcon className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            <div className="flex-1 min-w-0">
              <p className="flex items-center gap-1 text-sm font-medium">
                {w.githubTitle}
                <ExternalLink className="h-3 w-3 text-muted-foreground" />
              </p>
              <p className="mt-0.5 text-xs text-muted-foreground">{w.githubDesc}</p>
            </div>
          </a>

          {/* Internal contact */}
          <div className="flex items-start gap-3 rounded-xl border border-border bg-card p-4">
            <MessageCircle className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            <div>
              <p className="text-sm font-medium">{w.contactTitle}</p>
              <p className="mt-0.5 text-xs text-muted-foreground">{w.contactDesc}</p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
